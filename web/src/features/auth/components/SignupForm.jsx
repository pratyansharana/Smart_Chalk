...
export function SignupForm() {
  ...
  const {
    ...
    defaultValues: { displayName: '', email: '', password: '', role: ROLE.STUDENT },
  });
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      navigate(getDefaultRouteForRole(values.role), { replace: true });
    } catch (error) {
    ...
    <form className="glass-card grid gap-5 p-6" onSubmit={handleSubmit(onSubmit)}>
      {!firebaseReady && (
        <div className="flex gap-3 rounded-2xl border border-amber-400/20 bg-amber-500/10 p-3 text-sm text-amber-100">
          ...
          Add Firebase environment values before creating accounts.
        </div>
      )}
      <button
      ...
      <ReusableInput label="Full name" autoComplete="name" error={errors.displayName?.message} {...register('displayName')} />
      ...
      <label className="grid gap-2 text-sm font-semibold text-slate-100">
        Role
        <select className="apex-input" aria-invalid={Boolean(errors.role)} {...register('role')}>
          <option value={ROLE.STUDENT}>Student</option>
          <option value={ROLE.TEACHER}>Teacher</option>
        </select>
        {errors.role && <span className="text-xs text-red-300">{errors.role.message}</span>}
      </label>
